...
class DetectionLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=False):
        ...
        self.scaled_anchors = FloatTensor([(a_w / self.stride, a_h / self.stride) for a_w, a_h in self.anchors])  #anchor框对应原图的像素/特征图对应原图几个像素=anchor框对应特征图几个像素
    ...
    def forward(self, x, targets=None, input_dim=416, anchors=[], num_classes=80):
        ...
        prediction = x.view(num_samples, self.num_anchors, self.num_classes + 5, grid_size, grid_size).permute(0, 1, 3, 4, 2).contiguous() #划分新内存存储 
        ...
        #x预测的是相对于每个网格中心的偏移位置 参考坐标是网格左上角的坐标 现在加上每个网格左上角的绝对坐标 就得到x的绝对坐标(这里的坐标还是特征图的坐标不是映射到原图的坐标)
        ...
        predictions = torch.cat(
            (
                pred_boxes.view(num_samples, -1, 4)*self.stride, #从特征图的坐标乘上stride得到原图的坐标 (n,3,grid_size,grid_size,4) to (n,3*grid_size*grid_size,4)
                pred_conf.view(num_samples, -1, 1),
                pred_cls.view(num_samples, -1, self.num_classes)
            ), 2
        )  #输出(n,3*grid_size*grid_size,(4+1+80)*3=255)
        
        if targets == None:  # 没传标签 就是训练
            return predictions, 0
        else:
            final_predictions = torch.cat( #保留位置信息计算损失
            (
                pred_boxes.view(num_samples, self.num_anchors, grid_size, grid_size, 4) * self.stride, 
                pred_conf.view(num_samples, self.num_anchors, grid_size, grid_size, 1),
            ...
            anchor_boxes = FloatTensor(prediction[..., :4].shape)
            anchor_boxes[..., 0] = FloatTensor(prediction[..., 0].shape).fill_(0.5) + self.grid_x
            anchor_boxes[..., 1] = FloatTensor(prediction[..., 1].shape).fill_(0.5) + self.grid_y
            #(3,1) 2 (n,3,grid_size,grid_size)
            anchor_boxes[..., 2] = self.scaled_anchors[:, 0:1].view(1, self.num_anchors, 1, 1).repeat(num_samples, 1, grid_size, grid_size)
            anchor_boxes[..., 3] = self.scaled_anchors[:, 1:2].view(1, self.num_anchors, 1, 1).repeat(num_samples, 1, grid_size, grid_size)
...
class Darknet(nn.Module):
    ...
    def create_modules(self, blocks):
        ...
        for i, x in enumerate(blocks):
            ...
            if x['type'] == 'convolutional':
                ...
                #卷积块包括卷积层,BN层和激活函数
                conv = nn.Conv2d(prev_filters, filters, kernel_size, stride, pad, bias=bias)  #(输入通道数,输出通道数,卷积核大小,步长,pad,偏移)
    ...
    def forward(self, x, targets=None, CUDA=False): #targets是标签 代表需不需要计算误差
        modules = self.blocks
        outputs = {}  #存储每层输出的特征图
        yolo_outputs = []
        loss = 0
        ...
        for i, module in enumerate(modules):
            ...
            if module_type == 'convolutional' or module_type == 'upsample':  #这两种层torch有实现
                ...
                x, layer_loss = self.module_list[i][0](x, targets, input_dim, anchors, num_classes)
                loss += layer_loss
...
a=Darknet('cfg\\yolov3.cfg')                 
a.load_darknet_weights('weights\\infrared_yolov3_91_38.weights')
# a.save_darknet_weights('weights\\test.weights')                 
training_set = DIYDataset('D:\\Ubuntu_Server_Share\\REMOTE\\datasets\\coco_flir\\coco', 'train', mean_std_path=None
, cal_mean_std=False, transform=transforms.Compose([
    Normalizer(), Augmenter(), Resizer(416)]))
